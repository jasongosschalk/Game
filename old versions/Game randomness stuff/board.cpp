...
{
    ui->setupUi(this);
    //set up random numbers we can use throughout
    std::default_random_engine generator(SEED);   //generates a random unsigned int
    std::uniform_int_distribution<int> distribution(1,10);  //creates a uniform distribution
    auto randomLocation = std::bind(distribution, generator);
    ...
    std::uniform_int_distribution<int> distributionFive(1,5);
    int five; = distribution(generator);  // generates number in the range 1..6

    for (int i=0, j=numberAsteroids; i<j; ++i){
        int number = randomLocation();
        //int number = rand() % 10; //was 20
        if(number == 0){
            five = distribution(generator);  // generates number in the range 1..6
            number += five;
    ...
    int number = randomLocations(); //was 20
